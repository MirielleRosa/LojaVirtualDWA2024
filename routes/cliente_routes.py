...
async def get_carrinho(request: Request):
    ...
    if not pedido_carrinho or not itens_pedido:
        ...
        adicionar_mensagem_alerta(
            response,
            "Seu carrinho está vazinho. Adicione produtos"
        )
        return response
    
    total_pedido = sum(item.valor_produto * item.quantidade for item in itens_pedido)
    return templates.TemplateResponse(
        "pages/carrinho.html",
...
async def get_pagamento(request: Request, id_pedido: int = Path(...)):
    ...
    preference = {
        ...
        "payer": {
            "name": "Test",
            "surname": "Test",
            "email": "test_user_762512310@testuser.com",
        },
        "back_urls": {
